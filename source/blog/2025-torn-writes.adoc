...
////
There are three levels of the computing stack involved in writing to disk: the database, the operating system, and the storage device.  Each has their own unit of data upon which they operate atomically: the database's page size (typically >=4KB), the operating system's filesystem block size (typically 4KB), and the storage device's sector size (typically 512B or 4KB). When one of the sizes higher in the stack is larger than a size lower in the stack (a 16KB database page size vs a 512 disk sector size), it means that a database issuing one write to a page might be split into multiple writes to disk, and thus might not be performed atomically. Special attention is sometimes paid to torn writes in databases, because it's the only form of data corruption that's entirely permitted as part of a correctly functioning storage device and operating system.
////
...
[.aside]#sidenote:def[] As of 6.13, ext4 will return an error, and XFS won't. XFS {uri-xfs-fix}[will be fixed], and then checking `statx()` first won't be mandatory.  However, from side conversation with an ext4 dev, you aren't _supposed_ to rely on `pwritev2()` return codes, and you should check `statx()` first.#
