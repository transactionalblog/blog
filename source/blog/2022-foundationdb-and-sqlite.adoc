...
****
[^1]: There's been multiple attempts at "embedded innodb" over time, and none of them have ever stuck.  Innobase Oy {uri-innobase-innodb}[released an official one in 2009].  There was another attempt from the MySQL fork Drizzle, which relased {uri-haildb}[HailDB].  A GPL license was attached in both cases, as it was derived from MySQL, which I suspect contributed to is unpopularity.
****
