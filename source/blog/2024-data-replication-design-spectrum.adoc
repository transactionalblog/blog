...
There isn't one correct answer.  Compiling together data on what replication algorithm is used by OSS, Proprietary, and internal-only Proprietary databases, we see a distribution of:
...
[.aside]#sidenote:def[] And many shades of gray in between. cite:[Gryff] is cite:[ABD] for reads and writes, and cite:[EPaxos] for read-modify-write operations.#
